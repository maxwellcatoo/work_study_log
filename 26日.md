...
4.`git网站上的一个vue组价合集的仓库vuejs/awesome-vue
...
8.`vue中computed的用法
	# 在轮播图或有其他的翻页功能的组件中，翻页效果全部由一个methods中的一个方法gotoPage()来实现，该方法通过接收一个参数来达到翻到固定页的效果，而前一页和后一页的实现，并不单独设置方法，而是通过引用gotoPage()方法，通过使用computed对该函数传不同的参数值来实现。(这样子写的话，耦合度比较高)
